...
/* $Id: togl.c,v 1.15 2002/09/26 16:56:04 kberg Exp $ */
...
/*
 * $Log: togl.c,v $
 * Revision 1.15  2002/09/26 16:56:04  kberg
 * fixed a typo
 *
 * Revision 1.14  2002/09/21 02:00:42  kberg
 * Added prgamas to turn off optimizations so windows version doesn't crash
 * in optimized build
 * ---------------------------------------------------------
 *
 * Revision 1.13  2001/10/09 22:26:13  smr
 * Add #define for TK 8.3.3
 *
 * Revision 1.12  2001/07/24 22:12:22  kberg
 * Update on the overlay plane work around. (kberg)
 *
 * Revision 1.11  2001/07/02 17:56:30  srm
 * switched trunk over to branch non-lmeunier-rollfwd
 *
 * Revision 1.7.2.3  2001/06/14 15:06:45  srm
 * roll fwd to 2000-12-16
 *
 * Revision 1.10  2000/11/22 08:55:40  srm
 * Added yet another Tcl version to togl.
 * Note 8.3.{1,2} point to the 8.3.2 header file
 *
 * Revision 1.9  2000/06/06 10:17:59  magi
 * fix win32 compile
 *
 * Revision 1.8  2000/05/07 00:02:53  smr
 * Linux portability fixes.
 *
 * Revision 1.7  1999/01/12 17:35:26  magi
 * update win32 port
 *
 * Revision 1.6  1998/11/10 10:05:30  magi
 * change order of operations in destroy handler, so client's destroy
 * callback can still access widget fields like Togl->Ident
 *
 * Revision 1.5  1998/10/28 13:54:04  magi
 * cache static images as bitmap for fast redraws -- only render when necessary
 *
 * Revision 1.4  1998/06/17 01:37:10  magi
 * add per-togl support for timers
 *
 * Revision 1.3  1998/04/29 00:58:42  magi
...
#elif TK_MAJOR_VERSION==8 && TK_MINOR_VERSION==3 && TK_RELEASE_SERIAL==1
#  include "tkInt8.3.2.h"
#elif TK_MAJOR_VERSION==8 && TK_MINOR_VERSION==3 && TK_RELEASE_SERIAL==2
#  include "tkInt8.3.2.h"
...
Togl_Callback* Togl_GetCreateFunc( struct Togl *togl)
{
  return togl->CreateProc;
}
...
Togl_Callback* Togl_GetDisplayFunc( struct Togl *togl)
{
  return togl->DisplayProc;
}
...
Togl_Callback* Togl_GetReshapeFunc( struct Togl *togl)
{
  return togl->ReshapeProc;
}
...
{
   togl->TimerProc = proc;
   if (!TimerProc)    /* not already set */
     Tk_CreateTimerHandler( togl->Time, Togl_Timer, (ClientData)togl );
}


Togl_Callback* Togl_GetTimerFunc( struct Togl *togl)
{
  return togl->TimerProc;
}
...
Togl_Callback* Togl_GetDestroyFunc( struct Togl *togl)
{
  return togl->DestroyProc;
}
...
// turn off optimizations so release version doesn't crash
   ...
   if (togl->ShareContext && FindTogl(togl->ShareContext)) {
      ...
      if (togl->ShareList) {
         ...
         if (shareWith)
            ...
            shareCtx = None;
         togl->GlCtx = glXCreateContext(dpy, visinfo, shareCtx, GL_TRUE);
      }
      else {
...
static void ToglCmdDeletedProc( ClientData clientData )
{
   ...
   if (togl->OverlayCtx) {
	...
	Tcl_DeleteHashEntry(entryPtr);
      ...
      assert (togl->display && togl->OverlayCtx);
