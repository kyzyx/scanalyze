...
{
  typedef _Slist_iterator<_T, _T&, _T*>             iterator;
  typedef _Slist_iterator<_T, const _T&, const _T*> const_iterator;
  typedef _Slist_iterator<_T, _Ref, _Ptr>           _Self;

  typedef _T              value_type;
  typedef _Ptr            pointer;
  typedef _Ref            reference;
  typedef _Slist_node<_T> _Node;
...
template <class _T, class _Ref, class _Ptr>
inline _T* value_type(const _Slist_iterator<_T, _Ref, _Ptr>&) {
  return 0;
}
...
protected:
  _Slist_node<_T>* _M_get_node()
    { return _M_node_allocator.allocate(1); }
  void _M_put_node(_Slist_node<_T>* __p)
    { _M_node_allocator.deallocate(__p, 1); }

protected:
  typename _Alloc_traits<_Slist_node<_T>,_Allocator>::allocator_type
           _M_node_allocator;
...
class _Slist_alloc_base<_T,_Allocator, true> {
public:
  typedef typename _Alloc_traits<_T,_Allocator>::allocator_type
          allocator_type;
...
protected:
  typedef typename _Alloc_traits<_Slist_node<_T>, _Allocator>::_Alloc_type
          _Alloc_type;
  _Slist_node<_T>* _M_get_node() { return _Alloc_type::allocate(1); }
  void _M_put_node(_Slist_node<_T>* __p) { _Alloc_type::deallocate(__p, 1); }
...
struct _Slist_base
  : public _Slist_alloc_base<_T, _Alloc,
                             _Alloc_traits<_T, _Alloc>::_S_instanceless>
{
  typedef _Slist_alloc_base<_T, _Alloc,
                            _Alloc_traits<_T, _Alloc>::_S_instanceless>
          _Base;
...
protected:
  ...
  {
    _Slist_node<_T>* __next = (_Slist_node<_T>*) (__pos->_M_next);
...
protected:
  typedef simple_alloc<_Slist_node<_T>, _Alloc> _Alloc_type;
  _Slist_node<_T>* _M_get_node() { return _Alloc_type::allocate(1); }
  void _M_put_node(_Slist_node<_T>* __p) { _Alloc_type::deallocate(__p, 1); }
...
template <class _T, class _Alloc>
_Slist_node_base*
_Slist_base<_T,_Alloc>::_M_erase_after(_Slist_node_base* __before_first,
                                       _Slist_node_base* __last_node) {
  _Slist_node<_T>* __cur = (_Slist_node<_T>*) (__before_first->_M_next);
  while (__cur != __last_node) {
    _Slist_node<_T>* __tmp = __cur;
    __cur = (_Slist_node<_T>*) __cur->_M_next;
...
class slist : private _Slist_base<_T,_Alloc>
{
private:
  typedef _Slist_base<_T,_Alloc> _Base;
public:
  typedef _T                value_type;
...
public:
  friend bool operator== __STL_NULL_TMPL_ARGS (const slist<_T,_Alloc>& _SL1,
                                               const slist<_T,_Alloc>& _SL2);
...
public:
  ...
  void resize(size_type new_size, const _T& __x);
...
template <class _T, class _Alloc>
slist<_T,_Alloc>& slist<_T,_Alloc>::operator=(const slist<_T,_Alloc>& __x)
{
  if (&__x != this) {
...
template <class _T, class _Alloc>
void slist<_T, _Alloc>::assign(size_type __n, const _T& __val) {
...
void
slist<_T, _Alloc>::_M_assign_dispatch(_InputIter __first, _InputIter __last,
                                      __false_type)
{
  _Node_base* __prev = &_M_head;
  _Node* __node = (_Node*) _M_head._M_next;
...
template <class _T, class _Alloc>
bool operator==(const slist<_T,_Alloc>& _SL1, const slist<_T,_Alloc>& _SL2)
{
  typedef typename slist<_T,_Alloc>::_Node _Node;
...
template <class _T, class _Alloc>
inline bool operator<(const slist<_T,_Alloc>& _SL1,
                      const slist<_T,_Alloc>& _SL2)
{
...
template <class _T, class _Alloc>
inline void swap(slist<_T,_Alloc>& __x, slist<_T,_Alloc>& __y) {
  __x.swap(__y);
}
...
template <class _T, class _Alloc>
void slist<_T,_Alloc>::resize(size_type __len, const _T& __x)
{
...
template <class _T, class _Alloc>
void slist<_T,_Alloc>::remove(const _T& __val)
{
  _Node_base* __cur = &_M_head;
...
template <class _T, class _Alloc>
void slist<_T,_Alloc>::merge(slist<_T,_Alloc>& __x)
{
...
template <class _T, class _Alloc>
template <class _Predicate>
void slist<_T,_Alloc>::remove_if(_Predicate __pred)
{
  _Node_base* __cur = &_M_head;
  while (__cur->_M_next) {
...
void slist<_T,_Alloc>::unique(_BinaryPredicate __pred)
{
...
template <class _T, class _Alloc> template <class _StrictWeakOrdering>
void slist<_T,_Alloc>::sort(_StrictWeakOrdering __comp)
{
  if (_M_head._M_next && _M_head._M_next->_M_next) {
    slist __carry;
    slist __counter[64];
