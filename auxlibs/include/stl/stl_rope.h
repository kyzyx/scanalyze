...
# define __ROPE_DEFINE_ALLOC(_T, __name) \
        typedef typename \
          _Alloc_traits<_T,_Allocator>::allocator_type __name##Allocator; \
        _T* __name##_allocate(size_t __n) const \
          { return __name##Allocator(_M_data_allocator).allocate(__n); } \
        void __name##_deallocate(_T *__p, size_t __n) const \
                { __name##Allocator(_M_data_allocator).deallocate(__p, __n); }
  __ROPE_DEFINE_ALLOCS(_Allocator)
# undef __ROPE_DEFINE_ALLOC
};
...
#       endif
        ...
        // The basic rebalancing operation.  _Logically copies the
        // rope.  The __result has refcount of 1.  _The client will
        // usually decrement the reference count of __r.
        // The __result isd within height 2 of balanced by the above
        // definition.
   ...
   public:
        ...
        // Comparison member function.  _This is public only for those
...
template <class _CharT, class _Alloc>
const rope<_CharT, _Alloc>::size_type rope<_CharT, _Alloc>::npos = -1;
