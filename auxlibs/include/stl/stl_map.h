...
public:
  ...
  map(const map<_Key,_T,_Compare,_Alloc>& __x) : _M_t(__x._M_t) {}
  map<_Key,_T,_Compare,_Alloc>&
  ...
  _T& operator[](const key_type& __k) {
    return (*((insert(value_type(__k, _T()))).first)).second;
  }
  void swap(map<_Key,_T,_Compare,_Alloc>& __x) { _M_t.swap(__x._M_t); }
...
inline bool operator==(const map<_Key,_T,_Compare,_Alloc>& __x,
...
template <class _Key, class _T, class _Compare, class _Alloc>
inline void swap(map<_Key,_T,_Compare,_Alloc>& __x,
